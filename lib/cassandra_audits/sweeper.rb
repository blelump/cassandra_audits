    ...
    def before_create(audit)
      ...
      if controller.present?
        ...
        if superior_user.present?
          ...
          audit.audit_data.superior_data ||= controller.superior_user_data.to_json if controller.respond_to?(:superior_user_data)
        end
        audit.audit_data.remote_address = controller.try(:request).try(:ip) || ""
        if audit.audit_data.send(CassandraAudits.partition_key_name).blank?
          audit.audit_data.send("#{CassandraAudits.partition_key_name}=", CassandraAudits.partition_key_provider.call(controller))
        end
      else
#        raise CassandraAudits::PartitionKeyNotSpecified
      end
    end

    [:current_user, :superior_user].each do |method_name|
      define_method method_name do 
        controller.send(CassandraAudits.send("#{method_name}_method")) if controller.respond_to?(CassandraAudits.send("#{method_name}_method"))
      end
    end
